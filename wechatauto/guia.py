...
# 实测：宽屏 0.28、手机式竖屏 0.278（竖屏下“侧栏”= 整窗宽）——
# 取 0.25 留余量，两边都过；头像/角标（≤0.15）仍被滤掉。
...
class WeChatGUI:
    ...
    def get_sessions(self, zoomed: bool = False) -> List[Dict[str, object]]:
        ...
        名字列约占侧栏宽的 25%~100%（宽屏实测 0.28、手机式竖屏 0.278）；
        左侧 <25% 为头像/未读角标/图标，均按比例过滤，跨分辨率/档位一致。
        （注：竖屏档下“侧栏”= 整窗宽，阈值同样以 sidebar_right 为基准。）
